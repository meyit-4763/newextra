...
#
# Bu dosya < https://github.com/TheTeamVivek/YukkiMusic > projesinin bir parçasıdır
# ve "GNU v3.0 Lisans Anlaşması" altında yayımlanmıştır.
# Lütfen < https://github.com/TheTeamVivek/YukkiMusic/blob/master/LICENSE > adresini kontrol edin.
#
...
async def radio(client, message: Message):
    msg = await message.reply_text("Lütfen bir an bekleyin...")
    try:
        try:
            ...
            return await msg.edit_text(
                f"» Kullanıcıları davet etmek için {userbot.mention} asistanını {message.chat.title} sohbetine davet etme iznim yok."
            )
        if get.status == ChatMemberStatus.BANNED:
            return await msg.edit_text(
                text=f"» {userbot.mention} asistanı {message.chat.title} sohbetinde yasaklı.\n\n𖢵 ID : `{userbot.id}`\n𖢵 İsim : {userbot.mention}\n𖢵 Kullanıcı adı : @{userbot.username}\n\nLütfen asistanı yasaklamadan çıkarın ve tekrar oynatmayı deneyin..."
            )
            ...
            try:
                ...
                try:
                    ...
                    return await msg.edit(
                        f"{userbot.mention} asistanını {message.chat.title} sohbetine davet etme başarısız oldu.\n\n**Sebep:** `{ex}`"
                    )
            except Exception as ex:
        ...
        anon = await msg.edit_text(
            f"Lütfen bekleyin...\n\n{userbot.mention} asistanını {message.chat.title} sohbetine davet ediyorum."
        )
        try:
            await userbot.join_chat(invitelink)
            await asyncio.sleep(2)
            await msg.edit_text(
                f"{userbot.mention} başarıyla katıldı,\n\nyayın başlatılıyor..."
            )
